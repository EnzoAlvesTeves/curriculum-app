...
import com.example.curriculumapp.client.candidato.ExperienciaClient
import com.example.curriculumapp.client.candidato.dto.ExperienciaDTO
import com.example.curriculumapp.databinding.DialogExperienciaBinding
...
class ExperienciaListFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.candidato.observe(viewLifecycleOwner) { candidato ->
            candidato?.experiencias?.let { list ->
    ...
    private fun showAddDialog() {
        val dialogBinding = DialogExperienciaBinding.inflate(layoutInflater)
        ...
        AlertDialog.Builder(requireContext())
            ...
            .setPositiveButton("Salvar") { _, _ ->
                val novo = ExperienciaDTO(
                    empresa = dialogBinding.etEmpresa.text.toString(),
                    cargo = dialogBinding.etCargo.text.toString(),
                    resumo = dialogBinding.etResumo.text.toString(),
    ...
    private fun saveItem(item: ExperienciaDTO) {
        val candidatoId = viewModel.candidato.value?.id ?: return
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ExperienciaClient.api.criar(candidatoId, item)
                withContext(Dispatchers.Main) {
                    viewModel.loadCandidato()
                }
            } catch (e: Exception) {
                withContext(Dispatchers.Main) {
                    Toast.makeText(context, "Erro ao salvar experiência", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
    ...
    private fun deleteItem(item: CurriculoItem) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ExperienciaClient.api.deletar(candidatoId, id)
                withContext(Dispatchers.Main) {
                    viewModel.loadCandidato()
                }
            } catch (e: Exception) {
                withContext(Dispatchers.Main) {
                    Toast.makeText(context, "Erro ao deletar experiência", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
